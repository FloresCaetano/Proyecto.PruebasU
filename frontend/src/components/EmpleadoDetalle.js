	...
	return (
		<div className="panel">
			...
			{modo === "vendedores" && (
				<div>
					<section className="panel-section">
						<h3 className="section-heading">{entidadForm.id ? "Editar Vendedor" : "Crear Vendedor"}</h3>
						{error && <div className="error">{error}</div>}
						<form onSubmit={handleEntidadSubmit}>
							{entidadConfigs.vendedores.fields.map(field => (
								<div className="form-row" key={field.name}>
									<label>{field.label}:</label>
									<input
										className="input"
					...
					<section className="panel-section">
						...
						{loading ? <p>Cargando...</p> : items.length === 0 ? <p>No hay vendedores registrados.</p> : (
							<table className="panel-table">
								<thead>
									<tr>
										<th>ID</th>
